...
int main(int argc, char *argv[]) {
  ...
  while (distancia < 0 || distancia > 1)  {
    std::cout << "Introduzca 0 si desea Manhattan o 1 si desea Euclidea: ";
    std::cin >> distancia;
  ...
  int mov = 3;
  while (mov < 0 || mov > 1)  {
    std::cout << "Introduzca 0 si desea 4 Movimientos o 1 si desea 8 Movimientos: ";
    std::cin >> mov;
    if (mov < 0 || mov > 1) {
      std::cout << "Opción incorrecta. Vuelva a introducirla" << std::endl;
    }
  }
  std::vector<std::vector<int>> Camino = MainTaxi.CaminoMinimo4(distancia);       // Calculamos el camino mínimo entre las coordenadas de inicio y destino
  if (Camino.size() == 0) {                                             
